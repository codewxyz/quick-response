	...
	switch(type) {
		case 'global':
			io.on(ioDefaultEvents.connect, function (socket) {
    			...
    			// roomList['room'].online += 1;
        		// qrsCountUserOnline();
			    socketDefaultEvents(socket, 0);
			});
			break;
		case 'namespace':
...
function socketDefaultEvents(socket, type=1) {
	var user = getSocketUser(socket);
...
function qrsCountUserOnline(room='') {	
	if (room == '') {
		debugLog(roomList);
	    for (var idx in roomList) {    	
	    	...
	    	if (r.code == 'room')
		    	...
		    	nsList[r.org].in(r.code).clients(callbackGetClients(r.code));

	    }		
	} else {  	
    	var r = roomList[room];
    	nsList[r.org].in(room).clients((error, clients) => {
    		if (error) throw error;
		    var countObj = {
		    	count: clients.length
		    };
