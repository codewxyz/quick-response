    ...
    socket.on(orgEvents.join_room, function(f2_obj) {
        ...
        .then((getRoom) => {
            if (Object.keys(getRoom).length > 0) {
                f2_room = getRoom;
                return g_qrsModel.custom('smembers', g_qrsModel.getKeyUserDevice(f2_obj.username));
            } else {
...
function qrsSocketEvents(ns, socket) {
    ...
    //event a message deleted
    socket.on(roomEvents.message_deleted, function(msg) {
        io.emit(roomEvents.message_deleted, msg);
    });
